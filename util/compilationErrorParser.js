...
const parse = (stderr, regexMatchResult) => {
	...
	try {
		...
		/*
		 * If RegExp(`(${socketId}.c:\\d+:\\d+: error: )`) matched in the original stderr, ...
		 * ... header === "s-02c34e5658faf8e781.c:5:2: error: "
		 * If RegExp(`(${socketId}.c:\\d+:\\d+: )`) matched in the original stderr, ...
...
module.exports = (stderr, socketId) => {
	...
	try {
		 ...
		 *                        ^~~~
		 * s-6e8b4c6b70bbb4f18b.c:1:32: fatal error: iostream>in: No such file or directory
		 *  #include<iostream>int main() {}
		 *                                 ^
		...
		let substringWithErrorRegex = new RegExp(
			`(${socketId}.c:\\d+:\\d+: error: )`
		);
		// search for substring "s-02c34e5658faf8e781.c:5:2: fatal error"
		let substringWithFatalErrorRegex = new RegExp(
			`(${socketId}.c:\\d+:\\d+: fatal error: )`
		);
		// search for substring "s-02c34e5658faf8e781.c:5:2: "
